...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("Empty.mostRtweeted")
...
//    ((left union right) union that) incl elem
  ...
  def mostRetweetedAcc(tw: Tweet): Tweet =
    if(elem.retweets > tw.retweets) remove(elem).mostRetweetedAcc(elem)
    else
      remove(elem).mostRetweetedAcc(tw)
