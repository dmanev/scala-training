...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def traverse(idx: Int, maxTang: Float): Unit =
      if(idx < input.length) {
        val mt = max(input(idx)/idx, maxTang)
        output(idx) = mt
        traverse(idx + 1, mt)
      }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
     def traverse(idx: Int, maxTang: Float): Float =
      if(idx < until) traverse(idx + 1, max(input(idx)/idx, maxTang))
      else maxTang
